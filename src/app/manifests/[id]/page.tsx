  ...
  function load() {
    fetch(`/api/manifests/${id}`)
      ...
      .then((data: ManifestDetail) => {
        setManifest(data);
        setLandedCostInput(
          data.totalLandedCost != null ? String(data.totalLandedCost) : "",
        );
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-6">
        ...
        {isOwner &&
          (s.totalSoldRevenue != null ||
            s.totalSoldFees != null ||
            s.totalProfit != null) && (
            <section>
          ...
          <Card>
            <SectionHeader
              ...
              action={
                <Badge
                  tone={
                    evaluation.recommendation === "buy" ? "success" : "danger"
                  }
                  size="lg"
                >
            ...
            <dl className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <Metric
                label="Predicted max bid"
                value={
                  evaluation.maxBid != null
                    ? `$${evaluation.maxBid.toFixed(2)}`
                    : "—"
                }
              />
              {evaluation.maxBid != null &&
                    ...
                    <Metric
                      ...
                      value={
                        manifest.totalLandedCost > 0
                          ? `${(
                              ((evaluation.expectedNetContribution -
        ...
        {/* Owner only: landed cost is a dollar figure, and bulk discounting
            changes live eBay prices (its route is owner-only too). */}
        {isOwner && (
          <div className="grid gap-6 lg:grid-cols-2">
            <Card>
              <SectionHeader
                title="Total landed cost"
                description="What this whole load actually cost you, delivered."
              />
              ...
              {s.blendedCogsPerUnit != null ? (
                <p className="mt-3 text-sm text-muted-foreground">
                  ...
                  <strong className="text-foreground">
                    ${s.blendedCogsPerUnit.toFixed(4)}
                  </strong>{" "}
                  per unit (landed cost ÷ {s.totalReceivedUnits} units
                  received). Per-line weighted cost below accounts for each
                  item&apos;s share of the load&apos;s declared value, divided
                  by that line&apos;s good units received — damaged/expired and
                  still-missing units aren&apos;t counted.
                </p>
              ) : (
                <p className="mt-3 text-sm text-muted-foreground">
                  Enter a landed cost to see COGS per unit.
                </p>
              )}
            </Card>
          ...
          <Alert tone="warning" title="Scanned items not on this manifest">
            <ul className="flex flex-col gap-1">
              {manifest.unmatchedReceived.map((u) => (
                <li key={`r-${u.upc}`}>
                  Received {u.units} unit(s) of UPC {u.upc ?? "(none)"} — not on
                  the manifest.
                </li>
              ))}
              {manifest.unmatchedDamaged.map((u) => (
                <li key={`d-${u.upc}`}>
                  Marked {u.units} unit(s) damaged for UPC {u.upc} — not on the
                  manifest.
                </li>
              ))}
              {manifest.unmatchedDud.map((u) => (
                <li key={`x-${u.upc}`}>
                  Marked {u.units} unit(s) dud/unsellable for UPC {u.upc} — not
                  on the manifest.
                </li>
              ))}
              {manifest.unmatchedSold.map((u) => (
                <li key={`s-${u.upc}`}>
                  ...
                  {isOwner && u.revenue != null
                    ? ` ($${u.revenue.toFixed(2)} revenue)`
                    : ""}{" "}
                  — not on the manifest.
                </li>
              ))}
        ...
        <section>
          <SectionHeader
            title="Line items"
            action={<Badge>{manifest.lines.length}</Badge>}
          />
              ...
              return (
                ...
                >
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground tabular-nums">
                    {line.upc ?? "no UPC"}
                    {isOwner && line.retailPrice != null
                      ? ` · $${line.retailPrice.toFixed(2)} retail`
                      : ""}
                  </p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    ...
                    {line.missingUnits !== 0 && (
                      <Badge tone="danger">Missing {line.missingUnits}</Badge>
                    )}
                    {line.damagedUnits > 0 && (
                      <Badge tone="warning">Damaged {line.damagedUnits}</Badge>
                    )}
                    {line.dudUnits > 0 && (
                      <Badge tone="warning">Dud {line.dudUnits}</Badge>
                    )}
                    {line.soldUnits > 0 && (
                      <Badge tone="primary">Sold {line.soldUnits}</Badge>
                    )}
                  </div>
                  {isOwner && (
                    <p className="mt-2 flex flex-wrap gap-x-3 text-xs text-muted-foreground tabular-nums">
                      <span>
                        COGS{" "}
                        {line.weightedCogsPerUnit != null
                          ? `$${line.weightedCogsPerUnit.toFixed(4)}`
                          : "—"}
                      </span>
                      <span>
                        Revenue{" "}
                        {line.soldRevenue != null
                          ? `$${line.soldRevenue.toFixed(2)}`
                          : "—"}
                      </span>
                      <span
                        className={cn(
              ...
              <table className="w-full text-sm">
                <thead className="bg-background">
                  <tr className="border-b border-border text-left text-xs font-medium text-muted-foreground">
                    ...
                    {isOwner && (
                      <th className="px-3 py-3 text-right">COGS/unit</th>
                    )}
